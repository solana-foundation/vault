...
fn test_initialize_pausable_subscriptions_paused_false() {
    ...
    let user_before = get_token_account_amount(&svm.get_account(&user_token_account).unwrap());
    ...
    let vault_after =
        Vault::from_bytes(svm.get_account(&vault_pubkey).expect("vault exists").data()).unwrap();
    assert_eq!(vault_after.pending_async_requests, 1);
...
#[test]
fn test_update_paused_false_allows_deposit() {
    let (mut svm, authority, asset_mint, share_mint, user, vault_pubkey, pending_vault_pubkey) =
        setup(Some(true));
    ...
    .expect("deposit should succeed after unpausing");
