...
#[derive(Accounts)]
pub struct InitPausableSubscription<'info> {
    #[account(mut)]
...
pub fn handler(ctx: Context<InitPausableSubscription>) -> Result<()> {
    ...
    require!(
        !extensions::has_extension(tlv_data, ExtensionType::PausableSubcriptionsExtension),
