...
/**
 * Vault NAV is encoded as `assets per share × 10^decimals`. Format it for display.
 */
export function formatNav(nav: bigint | number, decimals: number): string {
    return formatTokenAmount(nav, decimals);
}
